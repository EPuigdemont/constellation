<div class="page-glitter-wrapper flex h-full flex-col gap-4 p-4 lg:p-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
        <h1 class="text-xl font-semibold text-[var(--theme-text)]">{{ __('Notifications') }}</h1>
    </div>

    {{-- Today's Important Dates --}}
    @if ($todayDates->isNotEmpty())
        <div>
            <h2 class="mb-2 text-sm font-semibold text-[var(--theme-accent)]">{{ __('Today\'s Dates') }}</h2>
                    ...
                    <div @class([
                        ...
                        <div class="flex items-center gap-3">
                            <button wire:click="toggleDateDone('{{ $date->id }}')"
                                    class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full transition-colors {{ $date->is_done ? 'bg-green-500/20 text-green-500' : 'bg-[var(--theme-accent)] text-white' }}">
                                @if ($date->is_done)
    ...
    {{-- Pending Reminders --}}
    <div>
        <h2 class="mb-2 text-sm font-semibold text-[var(--theme-accent)]">{{ __('Pending Reminders') }}</h2>
        @if ($pendingReminders->isEmpty())
            <p class="py-4 text-center text-sm text-[var(--theme-text-muted)]">{{ __('No pending reminders.') }}</p>
        @else
            <div class="flex flex-col gap-2">
                @foreach ($pendingReminders as $reminder)
                    <div class="flex items-center justify-between rounded-lg border border-[var(--theme-accent)] bg-[var(--theme-accent)]/5 p-3 transition-colors">
                        <div class="flex items-center gap-3">
                            <button wire:click="toggleReminderDone('{{ $reminder->id }}')"
                                    class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-[var(--theme-accent)] text-white transition-colors">
    ...
    @if ($upcomingDates->isNotEmpty())
        <div>
            <h2 class="mb-2 text-sm font-semibold text-[var(--theme-text-muted)]">{{ __('Upcoming Dates') }}</h2>
            <div class="flex flex-col gap-2">
                @foreach ($upcomingDates as $date)
                    <div class="flex items-center justify-between rounded-lg border border-[var(--theme-border)] p-3"
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <div class="text-sm font-medium text-[var(--theme-text)]">{{ $date->label }}</div>
                                <div class="text-xs text-[var(--theme-text-muted)]">
    ...
    @if ($completedReminders->isNotEmpty())
        <div>
            <h2 class="mb-2 text-sm font-semibold text-[var(--theme-text-muted)]">{{ __('Recently Completed') }}</h2>
            <div class="flex flex-col gap-2">
                @foreach ($completedReminders as $reminder)
                    <div class="flex items-center justify-between rounded-lg border border-[var(--theme-border)] p-3 opacity-50">
                        <div class="flex items-center gap-3">
                            <button wire:click="toggleReminderDone('{{ $reminder->id }}')"
                                    class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-green-500/20 text-green-500 transition-colors">
                                <flux:icon name="check" variant="outline" class="size-4" />
                            </button>
                            <div>
                                <div class="text-sm font-medium text-[var(--theme-text-muted)] line-through">
                                    {{ $reminder->title }}
                                </div>
                                <div class="text-xs text-[var(--theme-text-muted)]">
                                    {{ $reminder->remind_at->translatedFormat('j F Y, H:i') }}
