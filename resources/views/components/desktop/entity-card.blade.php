...
@php
    ...
    $isOwner = ($card['owner_id'] ?? null) === auth()->id();
    $ownerUsername = $card['owner_username'] ?? $card['owner_name'] ?? '';
@endphp
...
<div class="desktop-card-inner">
    ...
    @php
     ...
     $isShared = !$isOwner; // Entity is shared if the current user is not the owner
    @endphp
    ...
    @if($isShared)
        <div class="desktop-card-shared" title="{{ __('Shared with you') }}">
